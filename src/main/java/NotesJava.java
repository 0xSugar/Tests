...
Function<T, R>              Принимает 1 параметр, возвращает какой-то второй.. т.е. <Integer, String> - принимает число, возвращает String... можно написать x -> String.valueOf(x); и вернет строку из числа x
...
Если создавать именно Stream(), то его можно использовать отлько 1 раз и потом он закроется. Если нужно использовать много раз, то нужно использовать Supplier<Stream<String>> sup = () -> stream; и скорее всего его нужно потом закрыть
...
Stream<T> peek()	        Выполняет действие над каждым данным (т.е. когда элементы проходят через этот метод - можно задать какое-то действия, оно выполнится и данные пойдут дальше
...
Stream<R> map()	            Преобразовывает данные из одного типа в другой  | к примеру toUpperCase()
